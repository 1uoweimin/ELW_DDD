...
using FileService.WebAPI.Controllers.v2.Requests;
using FileService.WebAPI.Controllers.v2.Responses;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using Zack.ASPNETCore;

namespace FileService.WebAPI.Controllers.v2;
...
public class UploaderController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<ApiResponse<FileExistRsp>>> FileExist([FromBody] ApiRequest<FileExistReq> req)
    {
        string reqTime = req.ReqTime;
        long fileSizeInBytes = req.ReqData.FileSizeInBytes;
        string fileSHA256Hash = req.ReqData.FileSHA256Hash;

        var uploadedItem = await _fsRepository.FindFileAsync(fileSizeInBytes, fileSHA256Hash);
        if (uploadedItem == null)
            return ApiResponse<FileExistRsp>.Succeed(new(false, null));
        else
            return ApiResponse<FileExistRsp>.Succeed(new(true, uploadedItem.RemoveUrl));
    }
    ...
    public async Task<ActionResult<ApiResponse<UploadRsp>>> Upload([FromForm] ApiRequest<UploadReq> upload, CancellationToken cancellationToken)
    {
        /* Todo：
        ...
        var formFile = upload.ReqData.File;
        ...
        return ApiResponse<UploadRsp>.Succeed(new(uploadedItem.RemoveUrl));
    }
}